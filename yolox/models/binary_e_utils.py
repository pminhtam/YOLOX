...
class BinaryActivation(nn.Module):
    ...
    def forward(self, x):
        ...
        scaling_factor = torch.mean(torch.mean(abs(out_forward), dim=3, keepdim=True), dim=2, keepdim=True)
        scaling_factor = scaling_factor.detach()
        out_forward = scaling_factor * out_forward
        #out_e1 = (x^2 + 2*x)
        #out_e2 = (-x^2 + 2*x)
        out_e_total = 0
        mask1 = x < -1
...
class BinarizeConv2d(nn.Conv2d):
    def __init__(self, in_chn, out_chn, kernel_size=3, stride=1, padding=1,dilation=1,groups=1, bias=True,freeze = False):
        super(BinarizeConv2d, self).__init__(
                        in_chn, out_chn, kernel_size, stride, padding,dilation,groups, bias)
        self.shape = (out_chn, in_chn, kernel_size, kernel_size)
        self.weight = nn.Parameter(torch.rand(self.shape) * 0.001, requires_grad=True)

        self.act_inp = BinaryActivation()
    def forward(self, x):
        if x.size(1) != 3: # if input is not binary, then binarize it
            x = self.act_inp(x)
