#!/bin/bash
#SBATCH --job-name=yoloxOriVOC
#SBATCH --output=slurm_yolox_ori_voc_%A.out
#SBATCH --error=slurm_yolox_ori_voc_%A.err
#SBATCH --gpus=1
#SBATCH --ntasks=1
#SBATCH --nodes=1
...
export http_proxy=http://proxytc.vingroup.net:9090/
export https_proxy=http://proxytc.vingroup.net:9090/
rm -rf /home/tampm2/.conda/envs/yolox_ori
conda create --name yolox_ori python=3.7 --force
conda activate yolox_ori
...
python -m yolox.tools.train -n yolox-s -d 1 -b 32 -o  --data_dir ~/data/VOCdevkit/ --cache -expn yolox_s_ori --resume --logger wandb
